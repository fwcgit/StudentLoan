			...
			MyApplication.mainActivity.getHuankuan(new Handler.Callback() {
				...
				public boolean handleMessage(Message msg) {
					if (msg.arg1 == 1) {
						...
						com.qudaozhang.white.mode.activity.ZuPinActivity_.intent(getActivity()).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
					}
					return false;
				}
			});
...
//		}
		...
		com.qudaozhang.white.mode.activity.HuiGouActivity_.intent(getActivity()).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
