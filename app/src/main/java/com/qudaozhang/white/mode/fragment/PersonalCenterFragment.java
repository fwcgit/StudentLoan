...
import com.qudaozhang.white.utils.TitleBar.TitleCallBack;
...
//		com.studentloan.android.mode.activity.PersonalInfoActivity_.intent(this).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
		...
		//账户信息
		com.qudaozhang.white.mode.activity.AccountInfoActivity_.intent(this).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
	}
	@Click
	public void headImg(){
	...
	public void giveMoneyLayout(){
		//奖励金额
		com.qudaozhang.white.mode.activity.AwardActivity_.intent(this).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
	}
	
	@Click
	public void historyNoteLayout(){
		//历史记录
		com.qudaozhang.white.mode.activity.HistoryNotesActivity_.intent(this).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
	}
	
	@Click
	public void processLayout(){
		//进度查询
		com.qudaozhang.white.mode.activity.ProgressQueryActivity_.intent(this).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
	}
	@Click
	public void  msgLayout(){
		//消息
		com.qudaozhang.white.mode.activity.MsgActivity_.intent(this).flags(Intent.FLAG_ACTIVITY_NEW_TASK).start();
	}
	...
	public  void bankCarLayout(){
		...
		if(userInfo.blackList == 1 || userInfo.frozen == 1){
			MyApplication.mainActivity.getHuankuan(new Handler.Callback() {
				...
				public boolean handleMessage(Message msg) {
					if(msg.arg1 == 1){
						com.qudaozhang.white.mode.activity.BoundBankCardActivity_.intent(getActivity()).
			...
			com.qudaozhang.white.mode.activity.BoundBankCardActivity_.intent(this).
					flags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_NEW_TASK).start();
		}
